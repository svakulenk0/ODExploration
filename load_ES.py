...
class ESClient():
    ...
    def search(self, keywords, limit=N):
        ...
        result = self.es.search(index=self.index, size=limit, body={"_source": ["dataset.dataset_name", DATASET_LINK], "query": {"query_string": {"query": keywords, "default_operator": "AND", "analyzer": "german", "fields": list(FACETS.values())}}, 'highlight': {"pre_tags" : ["*"], "post_tags" : ["*"], 'fields': {'row.values.value': { "number_of_fragments" : 1 }}}})
